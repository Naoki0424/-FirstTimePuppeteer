...
// 認証情報jsonファイルを読み込む
const CREDIT = require('./vivid-carrier-302414-167d76f38d29.json');
...
(async () => {
    ...
    try {
        ...
        // シーモアのトップページへアクセス
        await page.goto(config.COMIC_TOP);

        // ログイン画面に遷移
        await toNextPage(page, '//*[@id="home"]/header/div[1]/div[3]/a[1]');

        // IDとパスワードを入力
        await page.type('input[name=email]', config.COMIC_ID, { delay: 100 });
        await page.type('input[name=password]', config.COMIC_PATHWORD, { delay: 100 });
        ...
        // スプレッドシートに出力
        setDataToSpreadsheet(config.SPREADSHEET_KEY, 0, outputCsvList);
...
async function getCsvText(outputCsvList, page) {
    ...
    // 価格
    cost = await getTextContentByXPath(page, ['//*[@id="purchase_form"]/div[1]/div[1]/div[3]/div[1]/div[2]/div/div[3]/div[2]', '//*[@id="purchase_form"]/div[1]/div[1]/div[3]/div[1]/div[2]/div/div/div[2]']);
    // 期限
...
function exportCSV(content){
    ...
    fs.writeFile('comicList.csv', formatCSV, 'utf8', function (err) {
      if (err) {
...
// スプレッドシートに出力する
const setDataToSpreadsheet = async (spreasheetKey, sheetIndex, outputCsvList) => {
    ...
    await doc.loadInfo();
    console.log('スプレッドシートのタイトル：', doc.title);
    // シートの取得
    const sheet = doc.sheetsByIndex[sheetIndex];
    // シートの内容をクリア
    await sheet.clear();
    // ヘッダー行を設定
    await sheet.setHeaderRow(outputCsvList[0]);
    // データ行を設定
    for(var row = 1; row < outputCsvList.length; row++) {
        await sheet.addRow(outputCsvList[row])
    }
